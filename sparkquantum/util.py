...
def get_conf(sc, config_str):
    ...
    if not c:
        if config_str not in ConfigDefaults:
            return None
        return ConfigDefaults[config_str]
...
def change_coordinate(rdd, old_coordinate, new_coordinate):
    ...
    if old_coordinate != MatrixCoordinateDefault:
        if old_coordinate == MatrixCoordinateMultiplier:
            rdd = rdd.map(
                lambda m: (m[1][0], m[0], m[1][1])
            )
        elif old_coordinate == MatrixCoordinateMultiplicand:
            rdd = rdd.map(
                lambda m: (m[0], m[1][0], m[1][1])
            )
        elif old_coordinate == MatrixCoordinateIndexed:
            rdd = rdd.map(
    ...
    if new_coordinate != MatrixCoordinateDefault:
        if new_coordinate == MatrixCoordinateMultiplier:
            rdd = rdd.map(
                lambda m: (m[1], (m[0], m[2]))
            )
        elif new_coordinate == MatrixCoordinateMultiplicand:
            rdd = rdd.map(
                lambda m: (m[0], (m[1], m[2]))
            )
        elif new_coordinate == MatrixCoordinateIndexed:
            rdd = rdd.map(
                lambda m: ((m[0], m[1]), m[2])
            )
        else:
