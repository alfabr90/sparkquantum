...
__all__ = ['PositionProbabilityDistribution']


class PositionProbabilityDistribution(ProbabilityDistribution):
    """Top-level class for probability distributions regarding the possible positions
    of a quantum system state."""

    def __init__(self, rdd, shape, num_variables, state, num_elements=None):
        """Build a top-level object for probability distribution regarding the possible
        positions of a quantum system state.
        ...
        if not is_state(state):
            self._logger.error(
                "'State' instance expected, not '{}'".format(
                    type(state)))
            raise TypeError(
                "'State' instance expected, not '{}'".format(
                    type(state)))
    ...
    def __str__(self):
        return 'Probability Distribution regarding the possible positions of a {}'.format(
            self._state)
...
def is_position_probability_distribution(obj):
    """Check whether argument is a :py:class:`sparkquantum.dtqw.math.statistics.probability_distribution.position_probability_distribution.PositionProbabilityDistribution` object.
    ...
    -------
    bool
        True if argument is a :py:class:`sparkquantum.dtqw.math.statistics.probability_distribution.position_probability_distribution.PositionProbabilityDistribution` object, False otherwise.

    """
    return isinstance(obj, PositionProbabilityDistribution)
