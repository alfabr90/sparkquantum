...
from sparkquantum.dtqw.mesh.grid.onedim.cycle import Cycle
...
class TestCycle(Base):
    def setUp(self):
        super().setUp()
        self.shape = (10, )
        self.mesh = Cycle(self.shape)
        self.mesh_perc = Cycle(self.shape,
    ...
    def test_str(self):
        self.assertEqual(str(self.mesh),
                         'Cycle grid with shape {}'.format(self.mesh.shape))
        self.assertEqual(str(self.mesh_perc),
                         'Cycle grid with shape {} and {}'.format(self.mesh_perc.shape,
