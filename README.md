...
| quantum.dtqw.walk.checkpointingFrequency | -1 | The frequency of checkpointing the states. A state will be checkpointed in every *n* steps. Considered only when `quantum.dtqw.walk.checkpointStates` evaluates to True. When -1, it never checkpoints the state. |
| quantum.dtqw.walk.dumpStates | False | Whether to dump to disk the states produced by each step of the walk. |
| quantum.dtqw.walk.dumpingFrequency | -1 | The frequency of dumping the states to disk. A state will be dumped every in *n* steps. Considered only when `quantum.dtqw.walk.dumpStates` evaluates to True. When -1, it never dumps the state. |
